...
class PlayingState(BaseState):
    ...
    def start_game(self):
        ...
        self.cook_timer = CountdownThread(starting_time)
                    ...
                    if elem.detect_collision_plate(self.plate):
                        if self.recipes:
                            ...
                            if next_ingredient.name == elem.name:
                                ...
                                self.recipes[0].ingredients.pop(0)
                                #gestione del piatto completato, si abilita a mandarlo in cucina
    ...
    def update(self, mouse_pos, screen_width, screen_height):
        ...
        if self.cook_timer:
            ...
            if current_timer_val >= self.cook_timer.clock_time:
                ...
                if self.recipes:
                    ...
                    if self.recipes:
                        self.current_time = self.recipes[0].time
                if len(self.recipes) == 0:
                    ...
                    self.cook_timer.stop_timer()
                    self.passed_time = 0.0
                    #completate tutte le ricette e si attende di passare al livello successivo o alla fine della partita
        ...
        if self.plate.position[1] < 0:
            self.cook_timer.reset_timer()
            #invio del messaggio al server per dire che un piatto è completato
            print("Il player avvisa il server")
            plate_total_score = self.recipes[0].score 
        ...
        for i in range(len(all_elements)):
            for j in range(i + 1, len(all_elements)):
                ...
                #Verifica se l'ingrediente spostato non è il prossimo ingrediente della ricetta 
        ...
        # mantiene gli ingredienti "cucinati" attaccati al piatto
